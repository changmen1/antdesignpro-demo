...
import { DeleteOutlined, ExclamationCircleOutlined, PlusOutlined, RollbackOutlined } from '@ant-design/icons';
...
import { allocatedUserList, authUserCancel, authUserCancelAll, authUserSelectAll, unallocatedUserList } from './service';
 ...
 * @param selectedRows
 */
const cancelAuthUserAll = async (roleId: string, selectedRows: User.IAddUser[]) => {
    ...
    <PageContainer>
      <div style={{ width: '100%', float: 'right' }}>
        <ProTable<User.IAddUser>
          ...
          request={(params) =>
            allocatedUserList({ ...params, roleId } as Role.RoleListParams).then((res) => {
              const result = {
                data: res.rows,
                total: res.total,
                success: true,
      ...
      <UserSelectorModal
        ...
        params={{ roleId }}
        request={(params) =>
          unallocatedUserList({ ...params } as Role.RoleListParams).then((res) => {
            const result = {
              data: res.rows,
